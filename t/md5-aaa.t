...
while (<DATA>) {
  ...
  if ( $module eq 'Digest::MD5' ) {
    $m = Digest::MD5::md5_hex($message);
  } else {
    $m = $c->md5_hex($message);
  }
#  $failed++ unless $m eq $hexdigest;
#  $failed++ unless $module->new->add(split(//, $message))->digest
#$failed++ unless $module->new->add($message)->digest

my $n = $module->new->add($message)->digest;
$failed++ unless $n eq pack("H*", $hexdigest);
